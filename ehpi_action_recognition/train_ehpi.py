...
def get_train_set(image_size: ImageSize):
    num_joints = 15
    left_indexes: List[int] = [3, 4, 5, 9, 10, 11]
    right_indexes: List[int] = [6, 7, 8, 12, 13, 14]

    datasets: List[EhpiDataset] = [
        EhpiDataset("/media/disks/beta/dataset_src/ehpi/ofp_record_2019_03_11_HSRT_30FPS",
        ...
        # Freilichtmuseum
        EhpiDataset("/media/disks/beta/dataset_src/ehpi/2019_03_13_Freilichtmuseum_30FPS",
                    transform=transforms.Compose([
                        RemoveJointsOutsideImgEhpi(image_size),
                        ScaleEhpi(image_size),
                        TranslateEhpi(image_size),
...
if __name__ == '__main__':
    ...
    # Train set
    train_set = get_train_set(image_size=ImageSize(1280, 720))
